...
namespace Balder.Core.Math
{
	internal class Gjk
	{
		...
		private Vector[][] edges = new Vector[][] { new Vector[4], new Vector[4], new Vector[4], new Vector[4] };
		private float maxLengthSq;
		private int simplexBits;
		private Vector[] y = new Vector[4];
		...
		public bool AddSupportPoint(ref Vector newPoint)
		{
			...
			{
				int num2 = (i & 7) - 1;
				Vector vector = this.y[num2] - newPoint;
		...
		private Vector ComputeClosestPoint()
		{
			float num3 = 0f;
			...
			for (int i = BitsToIndices[this.simplexBits]; i != 0; i = i >> 3)
			{
				...
				num3 += num4;
				zero += (Vector)(this.y[index] * num4);
				this.maxLengthSq = MathHelper.Max(this.maxLengthSq, this.yLengthSq[index]);
			}
			return (Vector)(zero / num3);
		}

		private static float Dot(ref Vector a, ref Vector b)
		{
